...
def render_crises_panel(state: GameState, clan_id: str) -> None:
    ...
    for crisis in crises:
        ...
        with st.container(border=True):
            ...
            if not clues:
                st.caption(
                    "Aucun détail fiable. Consacrez une action `Enquêter sur une crise` pour obtenir "
                    "des informations et faciliter les interventions suivantes."
                )
            for clue in clues:
                st.write(f"- {clue}")
            ...
            if crisis.faction == ANARCHS:
                st.caption(
                    "Approches généralement efficaces : infiltration ou négociation. La contenir par la "
                    "force est possible, mais plus difficile."
                )
            else:
                st.caption(
